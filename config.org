...
(defun my/insert--backlinks (&optional heading-func)
  (if-let* ((file-title (org-get-title))
            (raw-backlinks (org-roam-backlinks-get (org-roam-node-at-point) :unique t))
            (backlinks (seq-filter (lambda (elt)
                                     (not (equal
                                           (org-roam-node-title
                                            (org-roam-backlink-source-node elt))
                                           file-title)))
                                   raw-backlinks)))
      (save-excursion
        (when heading-func
      ...
      (unless skip-update-backlinks
        (let ((back-files (my/extract-org-roam-id origin-filename)))
          (dolist (back-file back-files)
            (message back-file)
            (with-current-buffer (dired-noselect back-file)
