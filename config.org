    ...
    (with-temp-buffer
      ...
      (if-let ((headline (org-find-exact-headline-in-buffer "总结")))
          (progn
            (goto-char headline)
            (org-cut-subtree)))
      (if-let ((headline (org-find-exact-headline-in-buffer "梦记")))
          (progn
            (goto-char headline)
            (org-cut-subtree)))
      (my/insert-backlinks)
...
(setq org-cliplink-curl-transport-arguments
      '("-L" "-x" "http://127.0.0.1:10809"
        "-H" "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:137.0) Gecko/20100101 Firefox/140.0"))
#+end_src
* 宏
** cond-let
...
设置 =orderless= 使用使用拼音首字母搜索，pyszm 最有用的一集。
